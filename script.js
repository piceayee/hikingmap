...
let trailMarkersData = []; // 儲存所有上傳照片的數據，用於匯出
let gpxLayer = null; // 用來存儲 GPX 軌跡圖層
let gpxHourlyMarkersData = []; // 🚩 修正：儲存 GPX 整點點位數據，用於 CSV 匯出
...
function addMarkerToMap(markerData) {
    ...
    if (isTrailMarker) {
        leafletTrailMarkers[markerData.id] = marker;
    } else {
         markers.push(marker);
    }
    ...
    listItem.querySelector(".go-to-marker").addEventListener("click", function() {
        const targetMarker = isTrailMarker ? leafletTrailMarkers[markerData.id] : marker;
        if(targetMarker) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (exportGpxHourlyBtn) {
                exportGpxHourlyBtn.disabled = false;
            }
        } catch (error) {
            ...
            const exportGpxHourlyBtn = document.getElementById("exportGpxHourlyBtn");
            if (exportGpxHourlyBtn) {
                exportGpxHourlyBtn.disabled = true;
            }
        }
    };
...
function parseAndDrawGpx(gpxText) {
    ...
    while (nextHourMs < endTimeMs) {
        ...
        // 加入找到的最接近點，並確保不重複
        if (closestPoint && gpxHourlyMarkersData[gpxHourlyMarkersData.length - 1].timeMs !== closestPoint.timeMs) {
...
// ... (其他 Photo upload, exportTrailData, exportTrailJson 函式保持不變) ...
...
async function handlePhotoUpload(event) {
    ...
    const promises = processedFiles.filter(p => p !== null).map(p => new Promise(resolve => {
        // EXIF.getData 仍從原始檔案讀取，確保 HEIC 的 EXIF 數據被保留
    ...
    const oldTrailRecords = trailMarkersData.map(item => ({
        isNew: false,
    ...
    if (allTrailRecords.length === 0) {
        alert("上傳的照片中沒有包含 GPS 資訊或時間戳的有效檔案。");
        event.target.value = "";
        return;
    }
    ...
    const reDrawPromises = allTrailRecords.map((item, index) => new Promise(resolve => {
        ...
        if (isNewFile) {
            ...
            lat = currentData.lat;
            lon = currentData.lon;
            rawDateStr = item.data.rawDateStr || currentData.time; 
            imageSource = currentData.image;
        }
        ...
        if (isNewFile) {
            ...
            reader.onload = function(e) {
                ...
                addMarkerToMap({
                    ...
                    categories: ["登山紀錄"], 
                    isTrailMarker: true,
                    latitude: finalMarkerData.lat, 
                    longitude: finalMarkerData.lon
                });
    ...
    if (trailMarkersData.length > 0) {
        exportTrailDataBtn.disabled = false;
        exportTrailJsonBtn.disabled = false;
        
        // 修正：自動移動到最後一張照片的位置並打開 Popup
        ...
        if (lastMarker) {
         ...
         exportTrailDataBtn.disabled = true;
         exportTrailJsonBtn.disabled = true;
    }
